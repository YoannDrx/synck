...
export interface LegacyPortfolioMetadata {
  id: number;
  slug: string;
  title: string;
  subtitle?: string;
...
export interface LegacyPortfolioItem extends LegacyPortfolioMetadata {
  description?: string;
}
...
const metadataCache = new Map<Locale, LegacyPortfolioMetadata[]>();
...
function getDescriptionsDir(locale: Locale) {
  return path.join(process.cwd(), "content", "portfolio", locale, "descriptions");
}
...
function loadLegacyMetadata(locale: Locale) {
  if (!metadataCache.has(locale)) {
    const metadataPath = path.join(process.cwd(), "content", "portfolio", locale, "metadata.json");
...
export function getLegacyPortfolioItem(slug: string, locale: Locale): LegacyPortfolioItem | null {
