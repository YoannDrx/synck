...
export default function LabelsPage({ params }: { params: Promise<{ locale: string }> }) {
  ...
  const columns: Column<Label>[] = [
    ...
    },
    {
      ...
      label: 'Ordre',
      sortable: true,
      render: (label) => <span className="text-sm text-white/70">{label.order}</span>,
    },
    {
      key: 'status',
      label: 'Statut',
      ...
      render: (label) => (
        <Badge
          ...
          className={
            label.isActive ? 'bg-[var(--brand-neon)] text-black' : 'border-white/30 text-white/50'
          }
        >
        ...
        <AlertDialogContent className="border-[var(--brand-neon)]/20 bg-black">
          <AlertDialogHeader>
            ...
            <AlertDialogDescription className="text-white/70">
              ...
              {labelToDelete ? (locale === 'fr' ? labelToDelete.nameFr : labelToDelete.nameEn) : ''}
