  ...
  return (
    <div className="space-y-6">
      ...
      <div>
        ...
        <p className="mt-2 text-white/50">Gérez les paramètres de sécurité de votre compte</p>
      </div>
      ...
      <Card className="border-white/10 bg-black">
        <CardHeader>
          <CardTitle className="flex items-center gap-3 text-white">
            {is2FAEnabled ? (
              <ShieldCheckIcon className="h-6 w-6 text-[var(--brand-neon)]" />
            ) : (
        ...
        <CardContent className="space-y-6">
          ...
          {twoFactorData && !is2FAEnabled && (
            <div className="space-y-6 rounded-lg border border-[var(--brand-neon)]/20 bg-[var(--brand-neon)]/5 p-6">
              <div>
                <h3 className="mb-4 text-lg font-semibold text-white">Configuration de la 2FA</h3>
                <ol className="space-y-4">
                  <li className="flex gap-3">
                    <span className="flex h-6 w-6 shrink-0 items-center justify-center rounded-full bg-[var(--brand-neon)] text-sm font-bold text-black">
                      1
                    </span>
                    <div>
                      <p className="font-medium text-white">
                  ...
                  <li className="flex gap-3">
                    ...
                    <div className="flex-1">
                      <p className="mb-4 font-medium text-white">Scannez ce QR code</p>
                  ...
                  <li className="flex gap-3">
                    ...
                    <div className="flex-1">
                      <p className="mb-3 font-medium text-white">Entrez le code de vérification</p>
          ...
          {!twoFactorData && (
            <div className="rounded-lg border border-white/10 bg-white/5 p-4">
              <h4 className="mb-2 font-medium text-white">Qu'est-ce que la 2FA ?</h4>
              <p className="text-sm text-white/70">
                L'authentification à deux facteurs ajoute une couche de sécurité supplémentaire à
                votre compte. Même si quelqu'un obtient votre mot de passe, il ne pourra pas se
                connecter sans le code généré par votre application d'authentification.
              </p>
            </div>
          )}
        </CardContent>
      ...
      <AlertDialog open={showDisableDialog} onOpenChange={setShowDisableDialog}>
        <AlertDialogContent className="border-[var(--brand-neon)]/20 bg-black">
          <AlertDialogHeader>
            <AlertDialogTitle className="text-white">Désactiver la 2FA ?</AlertDialogTitle>
            <AlertDialogDescription className="text-white/70">
              Êtes-vous sûr de vouloir désactiver l'authentification à deux facteurs ? Cela réduira
