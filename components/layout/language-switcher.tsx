...
export function LanguageSwitcher({ locale, dictionary }: LanguageSwitcherProps) {
  ...
  return (
    <div className="flex items-center gap-2 rounded-full border border-white/20 bg-black/40 px-3 py-1 text-[0.65rem] tracking-[0.4em] text-white/70 uppercase">
      ...
      {i18n.locales.map((targetLocale) => (
        <Link
          ...
          className={`rounded-full px-2 py-1 font-bold transition-colors ${
            targetLocale === locale
              ? 'bg-[var(--brand-neon)] text-[#050505]'
              : 'text-white/60 hover:text-[var(--brand-neon)]'
          }`}
