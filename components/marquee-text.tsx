...
export function MarqueeText({ items, className, speed = 1 }: MarqueeTextProps) {
  ...
  return (
    ...
    >
      <motion.div
        className="flex w-max items-center gap-8 px-6 py-6 text-xs font-black tracking-[0.6em] uppercase"
        style={{ x }}
      >
        {repeatedItems.map((item, index) => (
          <span key={`${String(index)}-${item}`} className="flex shrink-0 items-center gap-4">
            <span className="h-2 w-2 shrink-0 rounded-full bg-[var(--brand-neon)] shadow-[0_0_8px_rgba(213,255,10,0.6)]" />
            <span className="whitespace-nowrap text-[var(--color-text-primary)]">{item}</span>
          </span>
        ))}
      </motion.div>
    </div>
