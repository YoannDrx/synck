...
export function StatCard({
  ...
  return (
    <Card
      className={cn(
        '[border-color:var(--brand-neon,_#d5ff0a)]/20 bg-black transition-colors hover:[border-color:var(--brand-neon,_#d5ff0a)]/40',
        className
      )}
    >
      <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
        <CardTitle className="text-sm font-medium text-white/70">{title}</CardTitle>
        <Icon className="h-5 w-5 [color:var(--brand-neon,_#d5ff0a)]/50" />
      </CardHeader>
      <CardContent>
