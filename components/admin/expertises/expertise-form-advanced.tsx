...
import { ImageIcon, SaveIcon, RefreshCw, Plus, Trash, MoveUp, MoveDown } from "lucide-react";
...
export function ExpertiseFormAdvanced({
  ...
  const updateContentWithMetadata = useCallback((lang: "fr" | "en", newMetadata: any) => {
  ...
  const renderPreview = (lang: "fr" | "en") => {
    ...
    const sections = body
      .split("<!-- section:end -->")
      .map((s) => s.replace("<!-- section:start -->", "").replace("<!-- section:end -->", "").trim())
      .filter((s) => s.length > 0);

    // Map metadata to LogoGrid items
    const supportItems: LogoGridItem[] = frontmatter.supports?.map((s: any) => ({
        name: s.name,
        logo: s.logo,
    ...
    const labelItems: LogoGridItem[] = frontmatter.labels?.map((l: any) => ({
        name: l.name,
        logo: l.src,
        href: l.href
    })) ?? [];

    const companyItems: LogoGridItem[] = frontmatter.productionCompanies?.map((c: any) => ({
        name: c.name,
        logo: c.logo,
    ...
    return (
        <div className="bg-black text-white p-4 rounded-lg border border-white/10 overflow-hidden min-h-[500px] scale-[0.8] origin-top h-[800px] overflow-y-auto">
             ...
             <div className="mb-12 text-center">
                 ...
                 <p className="text-white/70 mt-4">{formData.translations[lang].description}</p>
             </div>
             ...
             <div className="mt-12 space-y-12">
                {supportItems.length > 0 && (
                    <LogoGrid items={supportItems} title="Supports" columns={3} showModal accentColor="lime" />
                )}
                {labelItems.length > 0 && (
                    <LogoGrid items={labelItems} title="Labels" columns={6} accentColor="cyan" />
                )}
                {companyItems.length > 0 && (
                    <LogoGrid items={companyItems} title="Production" columns={4} showModal accentColor="purple" />
                )}
             </div>
        </div>
    );
    ...
    <form onSubmit={(e) => { void handleSubmit(e); }} className="space-y-6">
        <div className="flex justify-between items-center">
            ...
            <div className="flex gap-2">
                ...
                <Button type="submit" disabled={isSubmitting} className="bg-lime-300 text-black hover:bg-lime-400 gap-2">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            ...
            <div className="space-y-6">
                 ...
                 <Tabs value={activeLocale} onValueChange={(v) => setActiveLocale(v as "fr" | "en")}>
                        ...
                        <TabsContent key={lang} value={lang} className="space-y-6 mt-4">
                             ...
                             <div className="space-y-2">
                                ...
                                <div className="text-xs text-white/50 mb-2">
                                    Utilisez `&lt;!-- section:end --&gt;` pour séparer les sections. Le frontmatter (YAML) en haut du fichier gère les métadonnées.
                                </div>
                                <MarkdownEditor 
                                    ...
                                    onChange={(val) => {
                                        setFormData({
                                            ...
                                            translations: {
                                                ...formData.translations,
                                                [lang]: { ...formData.translations[lang], content: val ?? "" }
                                            }
                                        });
                             ...
                             <Card className="bg-white/5 border-white/10">
                                <CardHeader>
                                    <CardTitle className="text-base text-white">Métadonnées (YAML Helper)</CardTitle>
                                </CardHeader>
                                <CardContent>
                                    <div className="grid grid-cols-2 gap-4">
                                        <div className="space-y-2">
                                            ...
                                            <Input 
                                                ...
                                                onChange={(e) => {
                                                    const newMeta = { ...metadata[lang], img1: e.target.value };
                                        ...
                                        <div className="space-y-2">
                                            ...
                                            <Input 
                                                ...
                                                onChange={(e) => {
                                                    const newMeta = { ...metadata[lang], img2: e.target.value };
                                                    updateContentWithMetadata(lang, newMeta);
                                                }}
                                                className="bg-black/20 h-8 text-xs text-white"
                                            />
                                    ...
                                    <div className="mt-4 text-xs text-white/40">
                                        Modifiez directement le YAML en haut de l'éditeur pour les listes complexes (Labels, Supports).
                                    </div>
                                </CardContent>
                             </Card>
                        </TabsContent>
