...
export function GalleryShell({
  ...
  return (
    ...
    >
          ...
          {stats.map((stat, index) => (
            <Fragment key={`${stat.label}-${String(index)}`}>
              ...
              <div className="text-right">
                <p className={cn('text-3xl font-black text-white', stat.valueClassName)}>
                  {stat.value}
                </p>
        ...
        <div className="mb-5 flex flex-col gap-4 sm:flex-row sm:items-center">
          <div className="relative max-w-md flex-1">
            <input
              ...
              className={cn(
                'w-full border-2 border-white/10 bg-black/30 px-4 py-3 text-sm text-white transition-all placeholder:text-white/30 focus:outline-none',
                search.inputAccentClassName ?? 'focus:border-[var(--brand-neon)]/50'
              )}
            />
            {search.value && (
              <button
                ...
                className={cn(
                  'absolute top-1/2 right-3 -translate-y-1/2 text-xs text-white/30 transition-colors',
                  search.clearButtonAccentClassName ?? 'hover:text-[var(--brand-neon)]'
                )}
      ...
      >
        {hasItems ? children : <div className="py-16 text-center">{emptyContent}</div>}
      </motion.div>
