...
function SingleOrb({
  ...
  return (
    <motion.div
      className={cn('absolute rounded-full', orb.position.x, orb.position.y, `bg-${orb.color}`)}
      style={{
        width: orb.size,
        height: orb.size,
...
export function MotionOrbs({ orbs = defaultOrbs, className, containerRef }: MotionOrbsProps) {
