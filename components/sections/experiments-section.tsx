...
type ExperimentsCopy = HomeDictionary["experiments"];

interface ExperimentsSectionProps {
  locale: Locale;
  copy: ExperimentsCopy;
}

export function ExperimentsSection({ locale, copy }: ExperimentsSectionProps) {
  ...
  useEffect(() => {
    async function fetchPosts() {
      try {
        const response = await fetch(`/api/blog?locale=${locale}&limit=3`);
        if (!response.ok) {
  ...
  if (loading) {
    return (
      <section id="experiments" className="rounded-[32px] border-4 border-white/15 bg-[#08080f] p-6 sm:p-10">
        ...
        <div className="mt-8 w-full overflow-hidden">
          <div className="flex gap-6 overflow-x-auto pb-4 text-white/90">
            {[1, 2, 3].map((id) => (
              <div key={id} className="min-w-[320px] rounded-[24px] border-4 border-white/10 bg-black/40 p-6 animate-pulse">
          ...
          {posts.map((post, index) => (
            ...
            >
              <div className="relative mb-4 aspect-[16/9] overflow-hidden rounded-lg">
                ...
                <div
                  className={`absolute inset-0 opacity-0 transition duration-300 group-hover:opacity-50 bg-gradient-to-br ${
                    index % 3 === 0
              ...
              <div className="space-y-3">
                ...
                <h3 className="text-xl font-bold leading-tight line-clamp-2 transition-all group-hover:bg-gradient-to-r group-hover:from-lime-300 group-hover:to-fuchsia-400 group-hover:bg-clip-text group-hover:text-transparent">
                  {post.title}
                </h3>

                {(post.subtitle || post.description) && (
                  <p className="text-sm text-white/70 line-clamp-2">{post.subtitle || post.description}</p>
                )}
