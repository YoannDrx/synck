...
const HeaderShapes = ({ theme }: { theme: CVTheme }) => (
  <Svg style={styles.headerShapes} viewBox="0 0 600 190">
    <Polygon points="0,0 180,0 0,190" fill="#11131C" opacity={String(0.8)} />
    <Polygon
      points="420,0 620,0 620,190"
      fill="#11131C"
      opacity={String(0.8)}
    />
    <Polygon
      points="520,0 620,0 620,110"
      fill={theme.primary}
      opacity={String(0.8)}
    />
    <Polygon
      points="0,140 60,190 0,190"
      fill={theme.primary}
      opacity={String(0.8)}
    />
    <Polygon
      points="140,0 340,0 240,120"
...
export const CVDocument = ({
  ...
  const customSidebarSections = sidebarSections.filter(
    (s) => s.type !== "education",
  );
  ...
  const contactItems = [
    data.email
      ...
      : null,
    data.phone
      ? { label: isFr ? "Téléphone" : "Phone", value: data.phone }
      : null,
    data.linkedInUrl
      ? {
          label: "LinkedIn",
    ...
    data.website
      ...
      : null,
    data.location
      ? { label: isFr ? "Localisation" : "Location", value: data.location }
      : null,
    ...socialFromLinks,
  ].filter((x) => x?.value) as {
    label?: string;
  ...
  const renderSidebarSection = (section: CVSection) => {
    ...
    return (
      <View
        key={section.id ?? sectionTitle}
        style={stylesWithTheme.sidebarBlock}
      >
        {sectionTitle && (
          <View>
            ...
            <View
              style={[
                stylesWithTheme.sidebarTitleBar,
        ...
        {section.items.map((item) => {
          ...
          return (
            <View key={item.id ?? it?.title} style={stylesWithTheme.listItem}>
              {it?.title && (
                <Text style={stylesWithTheme.listTitle}>{it.title}</Text>
              )}
              {it?.subtitle && (
                <Text style={stylesWithTheme.listSubtitle}>{it.subtitle}</Text>
              )}
              {it?.location && (
                <Text style={stylesWithTheme.listSubtitle}>{it.location}</Text>
              )}
              {item.startDate && (
      ...
      <View key={section.id ?? title} style={{ marginBottom: 20 }}>
        ...
        {(section.layoutType === "timeline" || section.type === "timeline") &&
          section.items.map((item, idx) => {
            ...
            return (
              <View
                key={item.id ?? it?.title}
                style={stylesWithTheme.timelineItem}
              >
                ...
                <View style={stylesWithTheme.timelineContent}>
                  {it?.title && (
                    <Text style={stylesWithTheme.expTitle}>{it.title}</Text>
                  )}
                  {it?.subtitle && (
                    <Text style={stylesWithTheme.expSubtitle}>
                      {it.subtitle}
                    </Text>
                  )}
                  ...
                  {it?.description && (
                    <Text style={stylesWithTheme.expDesc}>
                      {it.description}
                    </Text>
                  )}
                </View>
  ...
  return (
    <Document>
      <Page size="A4" style={stylesWithTheme.page} wrap>
        <View
          style={[
            stylesWithTheme.header,
            { backgroundColor: mergedTheme.header },
          ]}
        >
          ...
          <View style={stylesWithTheme.headerContent}>
            <View style={stylesWithTheme.headerLeft}>
              ...
              {headline && (
                <Text style={stylesWithTheme.headline}>{headline}</Text>
              )}
                  ...
                  if (item.isLink && item.href) {
                    return (
                      <Link
                        key={item.value}
                        src={item.href}
                        style={stylesWithTheme.contactItem}
                      >
                        {item.value
                          .replace(/^mailto:/, "")
                          .replace(/^tel:/, "")}
            ...
            <View>
              <Text style={stylesWithTheme.badge}>
                {isFr ? "Gestionnaire de droits" : "Rights manager"}
              </Text>
            </View>
          </View>
        ...
        <View style={stylesWithTheme.body}>
          ...
          <View style={stylesWithTheme.sidebar}>
            ...
            {technicalSkills.length > 0 && (
              <View style={stylesWithTheme.sidebarBlock}>
                ...
                {technicalSkills.map((skill) => {
                  ...
                  return (
                    <View
                      key={skill.id ?? st?.name}
                      style={stylesWithTheme.skillItem}
                    >
                ...
                <View style={stylesWithTheme.badgeRow}>
                  {softwareSkills.map((skill) => {
                    ...
                    return (
                      <Text
                        key={skill.id ?? st?.name}
                        style={stylesWithTheme.softBadge}
                      >
                        {st?.name}
                      </Text>
                    );
            ...
            {educationSection &&
              ...
              renderSidebarSection(educationSection)}
                ...
                <View style={stylesWithTheme.sectionHeader}>
                  <View
                    style={[
                      ...
                      {
                        backgroundColor:
                          experienceSection.color ?? mergedTheme.primary,
                      },
                    ]}
                  />
                  ...
                  return (
                    ...
                    >
                      ...
                      <View style={stylesWithTheme.timelineRail}>
                        ...
                        />
                        {!isLast && (
                          <View style={stylesWithTheme.timelineLine} />
                        )}
                      </View>
                      <View style={stylesWithTheme.timelineContent}>
                        {it?.title && (
                          <Text style={stylesWithTheme.expTitle}>
                            {it.title}
                          </Text>
                        )}
                        {it?.subtitle && (
