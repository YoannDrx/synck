...
const metadataEnPath = path.join(process.cwd(), 'content/projets/en/metadata.json')
...
function detectLabelFromPath(imagePath: string | null): string | null {
  ...
  if (imagePath.includes('/documentaires/little-big-story/')) return 'little-big-story'
  if (imagePath.includes('/documentaires/pop-films/')) return 'pop-films'
  if (imagePath.includes('/documentaires/via-decouvertes-films/')) return 'via-decouvertes-films'
