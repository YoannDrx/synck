...
const SOURCE_FILE = path.join(process.cwd(), 'content/expertises/fr/gestion-administrative-et-editoriale.md');
...
async function migrate() {
  ...
  for (const doc of data.documentaires) {
    ...
    fs.writeFileSync(path.join(descFrDir, `${slug}.md`), `---
title: "${doc.title}"
---

${doc.title}
`);
    fs.writeFileSync(path.join(descEnDir, `${slug}.md`), `---
title: "${doc.title}"
---
